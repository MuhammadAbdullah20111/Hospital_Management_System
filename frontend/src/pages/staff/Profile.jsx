...
import { User, Mail, Phone, Lock, Save, Key, Fingerprint, Send, Building2, Clock, Eye, EyeOff, Camera, Trash2 } from "lucide-react";
import { useFormik } from "formik";
import * as Yup from "yup";
import { toast } from "react-hot-toast";
import { getStaffProfileAPI, updateStaffProfileAPI, requestStaffPasswordOTPAPI, uploadStaffProfileImageAPI, deleteStaffProfileImageAPI, toggleStaff2FAAPI } from "../../api/staff/auth";
...
const StaffProfile = () => {
    ...
    const profileForm = useFormik({
        ...
        onSubmit: async (values) => {
            setIsSaving(true);
            try {
                const response = await updateStaffProfileAPI(values);
                if (response.success) {
    ...
    const passwordForm = useFormik({
        ...
        onSubmit: async (values, { resetForm }) => {
            setIsSaving(true);
            try {
                const response = await updateStaffProfileAPI({
    ...
    const handleRequestOTP = async () => {
        setIsSaving(true);
        try {
            const response = await requestStaffPasswordOTPAPI();
            if (response.success) {
    ...
    useEffect(() => {
        const fetchProfile = async () => {
            try {
                const response = await getStaffProfileAPI();
                if (response.success) {
                    setStaffData(response.staff);
                    setProfileImage(response.staff.profileImage || null);
                    profileForm.setValues({
                        name: response.staff.name || "",
                        email: response.staff.email || "",
                        phoneNumber: response.staff.phoneNumber || "",
                    });
                    setIs2FAEnabled(response.staff.isTwoFactorEnabled || false);
                }
            } catch (error) {
    ...
    const handleImageUpload = async (e) => {
        ...
        try {
            const response = await uploadStaffProfileImageAPI(formData);
            if (response.success) {
    ...
    const handleDeleteImage = async () => {
        ...
        setIsUploadingImage(true);
        try {
            const response = await deleteStaffProfileImageAPI();
            if (response.success) {
    ...
    const handleToggle2FA = async () => {
        ...
        try {
            ...
            const response = await toggleStaff2FAAPI({ enable: newValue });
            if (response.success) {
    ...
    return (
        <div className="max-w-4xl mx-auto py-8 px-4">
            <h1 className="text-2xl font-semibold text-slate-800 mb-8">My Account Settings</h1>

            <div className="space-y-12">
                {/* Employment Info (Read Only) */}
                <div className="bg-teal-50 border border-teal-100 rounded-xl p-6 flex flex-wrap gap-8">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="font-semibold text-slate-900">{staffData?.role}</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-3">
                        <div className="p-2 bg-white rounded-lg shadow-sm">
                        ...
                        <div>
                            ...
                            <p className="font-semibold text-slate-900">{staffData?.department || 'General'}</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-3">
                        <div className="p-2 bg-white rounded-lg shadow-sm">
                        ...
                        <div>
                            <p className="text-xs text-slate-500 uppercase font-bold tracking-wider">Shift</p>
                            <p className="font-semibold text-slate-900">{staffData?.shift || 'Flexible'}</p>
                        </div>
                    </div>
                </div>
                    ...
                    <div>
                        ...
                        <div className="mt-6 flex flex-col items-center sm:items-start">
                            <div className="relative group">
                                <div className="h-32 w-32 rounded-full overflow-hidden border-4 border-white shadow-lg bg-teal-50 flex items-center justify-center">
                                    {profileImage ? (
                                        ...
                                        <User className="h-16 w-16 text-teal-400" />
                                    )}
                                </div>
                                <label className={`absolute bottom-0 right-0 p-2 rounded-full bg-slate-800 text-white shadow-md cursor-pointer hover:bg-slate-900 transition-colors ${isUploadingImage ? 'opacity-50 cursor-not-allowed' : ''}`}>
                ...
                <section className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    <div>
                        ...
                        <p className="text-sm text-slate-500 mt-1">Change your portal password.</p>
                    </div>
                    <div className="md:col-span-2">
