...
import { createAppointmentAPI, getDoctorsAPI } from '../../../api/staff/appointments';
...
const StaffCreateAppointment = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                const [patientsRes, staffRes, catRes] = await Promise.all([
                    getAllPatientsAPI(),
                    getDoctorsAPI(),
                ...
                if (staffRes.success) {
                    ...
                    if (isDoctor) {
                        formik.setFieldValue('doctorId', String(loggedInUserId));
                    }
                }
                if (catRes.success) {
    ...
    const handleBillingSubmit = async (billingData) => {
        ...
        try {
            ...
            // 2. Create Transaction if category is selected or if they proceed with payment
    ...
    const discountAmt = parseFloat(formik.values.discount) || 0;
    const netAmount = Math.max(0, baseFee - discountAmt);
    ...
    return (
        <div className="max-w-4xl mx-auto pb-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="flex items-center gap-4 mb-8">
                <button
                    onClick={() => step === 2 ? setStep(1) : navigate('/staff/appointments')}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div className="md:col-span-1">
                                <SearchableSelect
                                    ...
                                    options={patients.map(p => ({ value: p.id, label: p.name, subLabel: `MR: ${p.mrNumber}` }))}
