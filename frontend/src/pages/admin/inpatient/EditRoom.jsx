...
import { getRoomByIdAPI, updateRoomAPI, getAllWardsAPI } from '../../../api/admin/inpatient';
...
const EditRoom = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const [roomRes, wardRes, categoryRes] = await Promise.all([
                    getRoomByIdAPI(id),
                    getAllWardsAPI(),
                ...
                if (roomRes.success) {
                    const { roomNumber, wardId, floor, categoryId, isActive } = roomRes.room;
                    setFormData({
                        roomNumber,
                        wardId,
                        floor: floor || '',
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await updateRoomAPI(id, data);
            if (response.success) {
    ...
    return (
        <div className="max-w-4xl mx-auto pb-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="flex items-center gap-4 mb-8">
                ...
                <div>
                    <h1 className="text-2xl font-bold text-slate-800">Edit Room</h1>
                    <p className="text-slate-500">Update room details and assignment</p>
                </div>
            </div>
