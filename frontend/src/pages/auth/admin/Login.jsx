...
import { adminLoginAPI, verifyAdmin2FAAPI } from '../../../api/admin/auth';
...
const AdminLogin = () => {
    ...
    const handleVerify2FA = async (values, { setSubmitting }) => {
        try {
            const response = await verifyAdmin2FAAPI({ email: authEmail, otp: values.otp });
            if (response.success) {
                ApiService.saveToken(response.token);
                localStorage.setItem('user', JSON.stringify(response.admin));
                localStorage.setItem('role', response.admin.role);
    ...
    return (
        <div className="flex flex-col w-full p-8 rounded-2xl shadow-xl bg-white">
            <div className="mb-8 text-center">
                <h2 className="text-3xl font-bold text-teal-600 dark:text-teal-500">Admin Login</h2>
                <p className="text-slate-500 dark:text-slate-400">Sign in to access control panel</p>
            </div>
                    ...
                    <Form className="flex flex-col gap-6">
                        ...
                        <div>
                            <div className="flex justify-between items-center mb-2">
                                ...
                                <Link to="/auth/admin/forgot-password" size="sm" className="text-sm text-teal-600 hover:text-teal-700 font-medium">Forgot password?</Link>
                            </div>
