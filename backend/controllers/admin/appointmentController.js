...
export const updateAppointment = asyncHandler(async (req, res) => {
    ...
    // If appointment is completed, create a transaction
    if (updateData.status === 'COMPLETED') {
        try {
            const appointment = await Appointment.findById(Number(id));
            const doctor = appointment.doctor;
            
            // Check if a transaction for this appointment already exists
            const existingTx = await prisma.transaction.findUnique({
                where: { appointmentId: appointment.id }
            });

            if (!existingTx) {
                // Find 'Appointment' category
                const categories = await prisma.transactionCategory.findMany({ where: { name: 'Appointment' } });
                ...
                await prisma.transaction.create({
                    data: {
                        ...
                        amount: doctor.consultationFee || 500, // Fallback to 500 if not set
                        ...
                        notes: `Consultation fee for Appointment #${appointment.id}`
                    }
                });
            }
        } catch (transError) {
            // Trade-off: Non-blocking billing failure so appointment still marks as completed, 
            // but log is recorded for manual reconciliation
            console.error("Failed to create transaction for completed appointment:", transError);
        }
    }
